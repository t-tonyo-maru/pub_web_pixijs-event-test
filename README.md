...
- グラフ描画: Chart.js
  - サンプル: `/src/ts/modules/createChart/createChart.ts`
...
- ブラウザ幅リサイズ処理: (プレーン TypeScript)
  - サンプル: `/src/ts/modules/setWindowResizeEvent/setWindowResizeEvent.ts`
- MediaQueryList オブジェクトを利用したメディアクエリマッチ: (プレーン TypeScript)
...
- Rust の Result 型を、Map を用いて擬似的に再現: (プレーン TypeScript)
  - サンプル: `/src/ts/modules/createResultMap/createResultMap.ts`
- Intl.Segmenter（※2022.09 時点で FireFox は未サポート）を利用した文字列カウント関数: (プレーン TypeScript)
